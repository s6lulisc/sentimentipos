...
def get_ipo_df(file_path):
    ...
    ipo['trade_date'] = pd.to_datetime(ipo['trade_date'], errors='coerce').dt.strftime('%Y-%m-%d')

    columns_to_drop = ['lead_jlead_mangr', 'offr_prc_pct_rtrn', '$_chg_open',
                       '$_chg_close', 'star_ratn', 'del']
